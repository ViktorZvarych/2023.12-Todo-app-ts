...
export default class TodoApp {
  ...
  private _addNewArticleToDocBody(): void {
  ...
  private _getDeletedTodoListFromLS(): void {
    const deletedTodoListFromLS = localStorage.getItem(
      this.todoAppName + 'deleted'
    );
  ...
  private _setDeletedTodoListToLS(): void {
    localStorage.setItem(
      this.todoAppName + 'deleted',
      JSON.stringify(this._deletedTodoList)
    );
  }
  ...
  private _initTodoList(): void {
    ...
    const todoOl = document.getElementById(
      'todo-ol' +
        '_' +
        this.todoAppName
          .split(' ')
    ...
    this._todoList.forEach(({ id, title, addDate }) => {
      ...
      deleteTodoLiButton.classList.add(
        'deleteTodoItem-button',
        'btn',
        'btn-primary'
      );
      deleteTodoLiButton.addEventListener('click', () => {
        this._deletedTodoList.push(
          this._todoList.filter((item) => item.id === id)[0]
        );
      ...
      editTodoLiButton.classList.add(
        'editTodoItem-button',
        'btn',
        'btn-primary'
      );
      ...
      editTodoLiButton.addEventListener('click', () => {
        ...
        editTodoForm.onsubmit = (e) => {
          ...
          if (e.target !== null) {
            const todoInput = e.target['editTodo-input'] as HTMLInputElement;
            console.log(todoInput.value);
            todoP.innerText = todoInput.value;
            
            this._todoList.forEach(
              (item) =>
                (item.title = item.id === id ? todoInput.value : item.title)
            );
          }            
  ...
  private _initAddTodoForm() {
    ...
    const todoForm = forms[
      'addTodo-form' + '_' + this.todoAppName
    ] as HTMLFormElement;
  ...
  private _initDeletedTodoList(): void {
    this._getDeletedTodoListFromLS();
    const deletedTodoOl = document.getElementById(
      'deletedTodo-ol' +
        '_' +
        this.todoAppName
          .split(' ')
    ...
    this._deletedTodoList.forEach(({ id, title }) => {
      ...
      restoreTodoLiButton.classList.add(
        'todo-button',
        'restoreTodoLi-button',
        'btn',
        'btn-primary'
      );
      restoreTodoLiButton.onclick = () => {
        this._todoList.push(
          this._deletedTodoList.filter((item) => item.id === id)[0]
        );
        this._deletedTodoList = this._deletedTodoList.filter(
          (item) => item.id !== id
        );
      ...
      removeTodoLiButton.classList.add(
        'todo-button',
        'removeTodoLi-button',
        'btn',
        'btn-primary'
